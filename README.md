...
如果多次保存后出现重复快照，可以点击 `清理重复 auth 快照`。工具会按 `auth.json` 文件内容计算指纹，只删除由 `保存当前 auth.json` 生成、且内容完全相同的旧快照；完整恢复备份不会因为 `auth.json` 重复而被删除。
...
如果只想恢复认证状态，可以选择含有 `auth.json` 的备份，然后点击 `恢复 auth.json`。这个操作只把备份里的 `auth.json` 写回 Codex root，不会改 SQLite、JSONL 或聊天记录。
...
这种快照只用于 `恢复 auth.json`，不能用于 `回滚恢复设置`。
...
界面里的 `清理重复 auth 快照` 是另一种更窄的清理方式：它只检查 `保存当前 auth.json` 生成的认证快照，保留每一种不同 `auth.json` 内容的最新一份，删除内容完全相同的旧副本。它不会删除完整恢复备份。
